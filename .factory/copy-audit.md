...
| 6 | Move forecast dates after actual finishes. | Pass |

No sentence exceeds 22 words. No sentence contains a banned marketing word.

## Headings, controls, and status fragments

`Forecast dates · built for small trade crews`, `Move forecast dates after actual finishes`, `Your jobs`, `Show active jobs`, `Show archived jobs`, `Set the first forecast date`, `How it works`, `Built for forecast dates, not dispatch`, `Track up to five active jobs`, `Try it with sample data`, `Start your first job`, `Add a job`, and `Open data settings` name their subject or result.

## Terminology

| Concept | One term used |
| --- | --- |
| Planned date shown by the app | forecast date |
| Day work really completed | actual finish |
| Ordered unit of work | step |
| Work record | job |
| Message for the customer | client update |
| Calendar exception | non-working date |

`Actuals Job Sequencer` appears only as the product name. User-facing copy does not use “working dates”, “job dates”, “local-first”, “finish-to-start”, or “Gantt”.
