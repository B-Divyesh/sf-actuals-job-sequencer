...
const metadata: Record<Route, { title: string; description: string }> = {
  home: { title: 'Actuals Job Sequencer — move forecast dates', description: 'Move forecast dates after an actual finish. Built for small trade crews and works offline after the first visit.' },
  demo: { title: 'Demo — Actuals Job Sequencer', description: 'Try a sample job without changing your jobs. See a late actual finish move later forecast dates.' },
...
function hero(): string {
  if (demoMode) return `<section class="hero demo-hero" aria-labelledby="page-heading"><div class="hero-copy"><p class="edition">Sample job · late rough-in</p><h1 id="page-heading">Move forecast dates after actual finishes</h1><p class="hero-dek">See the moved handover date and the client update below.</p></div></section>`;
  return `<section class="hero" aria-labelledby="page-heading"><div class="hero-copy"><p class="edition">Forecast dates · built for small trade crews</p><h1 id="page-heading">Move forecast dates after actual finishes</h1><p class="hero-dek">For small trade crews when a late step changes the forecast dates you gave a client.</p><div class="hero-actions"><a class="button primary" href="/demo/" data-route="demo">Try it with sample data</a><button class="button" data-action="add-job">Start your first job</button></div><p class="action-note">See a late rough-in move the handover date.</p></div><ul class="hero-facts" aria-label="Product facts"><li>Works offline after the first visit.</li><li>Jobs stay in this browser.</li><li>Five active jobs. No account.</li></ul></section>`;
}
...
function renderApp(): void {
  ...
  app.innerHTML = `${sharedHeader()}${demoMode ? demoBanner() : ''}${hero()}
    ...
    <div class="app-shell"><aside class="job-rail" aria-labelledby="jobs-heading"><div class="rail-head"><h2 id="jobs-heading">Your jobs</h2><span>${data.jobs.filter((item) => item.status === 'active').length}/${activeLimit()}</span></div><div class="mode-tabs" aria-label="Job list filter"><button data-action="rail-mode" data-mode="active" aria-pressed="${railMode === 'active'}">Show active jobs</button><button data-action="rail-mode" data-mode="archived" aria-pressed="${railMode === 'archived'}">Show archived jobs</button></div>${jobs.length ? `<ul class="job-list">${jobs.map(renderJobPick).join('')}</ul>` : `<p class="all-clear">No ${railMode} jobs. Start one to add its forecast dates.</p>`}<div class="rail-actions"><button class="button primary" data-action="add-job">Add a job</button><button class="button" data-action="settings">Open data settings</button></div></aside>
...
function makeClientMessage(job: Job, steps = scheduleJob(job, data.settings)): string {
  ...
  return `${job.client ? `Hello ${job.client}` : 'Hello'} — a forecast date update for ${job.name}.\n\n${lines.join('\n')}\n\nCurrent forecast job finish: ${finish ? formatDate(finish, 'long') : 'to be confirmed'}. These forecast dates use current estimates and working days. Please reply if you need to talk it through.`;
}

function explainSections(): string {
  return `<section class="explain" aria-labelledby="how-heading"><h2 id="how-heading">How it works</h2><ol><li><strong>Add the ordered steps.</strong><span>Give each step a working-day estimate.</span></li><li><strong>Record the actual finish.</strong><span>The next forecast starts after the work finished.</span></li><li><strong>Send the changed forecast dates.</strong><span>Copy the client update after checking it.</span></li></ol></section><section class="limits" aria-labelledby="limits-heading"><div><h2 id="limits-heading">Built for forecast dates, not dispatch</h2><p>It does not plan routes, payroll, GPS, or whole projects. Forecast dates remain your estimates.</p></div><div><h2>Track up to five active jobs</h2><p>Archive a finished job before adding another. Restore it when there is room.</p><button class="button" data-action="settings">Open data settings</button></div></section>`;
}
...
async function shareMessage(job: Job): Promise<void> { try { await navigator.share({ title: `${job.name} forecast date update`, text: makeClientMessage(job) }); } catch (error) { if ((error as DOMException).name !== 'AbortError') showToast('Sharing was unavailable. Copy the client update instead.'); } }
